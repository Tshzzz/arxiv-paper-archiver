...
def main() -> None:
    ...
    title = str(metadata.get("title", "")).strip() or args.arxiv_id
    api_key = get_bigmodel_api_key(args.api_key)
    local_pdf = pdf_path(args.archive_dir, args.arxiv_id, title)
    ...
    md_path = ocr_markdown_path(args.archive_dir, args.arxiv_id, title=title)
    md_path.write_text(md_results.rstrip() + "\n")
    write_json(ocr_response_path(args.archive_dir, args.arxiv_id, title=title), response)
    ...
    if isinstance(files, dict):
        ...
        files["ocr_response"] = str(ocr_response_path(args.archive_dir, args.arxiv_id, title=title))
    write_json(Path(local_pdf).parent / "metadata.json", metadata)
        ...
        json.dumps(
            {
                ...
                "ocr_response": str(ocr_response_path(args.archive_dir, args.arxiv_id, title=title)),
            },
            ensure_ascii=False,
            indent=2,
        )
